...
class EquipeAjuda():
    ...
    def __init__(self, status):      
        ...
        if status == "DUVIDA_OUTRA_COISA":
            self.nome_tarefa = "ajuda_sistema"

    @agent
    def ajudante(self) -> Agent:
        return Agent(
            config=self.agents_config["ajudante"],
            llm=self.llm,
    ...
    def classificar_solicitacao(self) -> Task:      
        return Task(
            config=self.tasks_config[self.nome_tarefa],            
        )
